...
var Appointments = function (appointment) {
  ...
  this.dealerProfileId = appointment.dealerProfileId;
  this.dealerName = appointment.dealerName;
...
Appointments.findAndSearchAll = async (
  ...
  let whereCondition = `${
    search ? `a.dealerName LIKE '%${search}%'` : ""
  }`;
  ...
  const searchData = await executeQuery(
    `SELECT a.id, a.dealerName, a.appointmentDateTime, a.createdDate,a.profileId,a.isCancelled,p.FirstName,p.LastName,p.ProfilePicName,p.Username FROM appointments as a left join profile as p on p.ID = a.profileId  ${
      whereCondition ? `WHERE ${whereCondition} ` : ""
...
Appointments.getPractitionerAppointments = async (id) => {
  const query =
    "select a.*,p.Username,p.FirstName,p.LastName,p.ProfilePicName from appointments as a left join profile as p on p.ID = a.profileId where a.isCancelled = 'N' and a.dealerProfileId = ?";
  const values = [id];
...
Appointments.getUserAppointments = async (id) => {
  const query =
    "select a.*,p.Username,p.FirstName,p.LastName,p.ProfilePicName from appointments as a left join profile as p on p.ID = a.dealerProfileId where a.isCancelled = 'N' and a.profileId = ?";
  const values = [id];
  const appointmentList = await executeQuery(query, values);
  if (appointmentList) {
    return appointmentList;
